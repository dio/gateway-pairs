...
### Customizing `envoy-gateway-config` per pair

The `envoy-gateway-config` ConfigMap in `tr-system-{i}` is generated by
`gateway-helm` from the `config.envoyGateway` values block. Every field in
the `EnvoyGateway` API is reachable via `--set gateway-helm.config.envoyGateway.*`
or via a `gateway-helm:` block in a values file passed to `gwp pair install`.

The full `EnvoyGateway` API reference is at
`https://gateway.envoyproxy.io/docs/api/extension_types/#envoygateway`.

#### Fields managed by `gwp pair install` (do not override manually)

| Field | Path | Why managed by CLI |
|---|---|---|
| `controllerName` | `gateway.controllerName` | Derived from pair identity; must be unique |
| `watch.namespaces` | `provider.kubernetes.watch.namespaces` | Derived from pair namespaces |
| `watch.type` | `provider.kubernetes.watch.type` | Always `Namespaces` for static pairs |
| `deploy.type` | `provider.kubernetes.deploy.type` | Always `GatewayNamespace` |

#### Fields operators can tune per pair

These go in `eg-pair/values.yaml` under `gateway-helm.config.envoyGateway`
or passed as `--set` flags at install time.
...
gateway-helm:
  config:
    envoyGateway:
      runtimeFlags:
        enabled:
        - XDSNameSchemeV2    # required for transit dynamic module address scheme
```
  ...
  config:
    envoyGateway:
      provider:
        ...
        kubernetes:
          rateLimitDeployment:
            replicas: 0           # disable rate limit sidecar if not needed
          shutdownManager:
            image:
              repository: custom/shutdown-manager
              tag: v1.0
```

#### The `gwp pair install --config-values` flag (planned)

For pairs that need significant `EnvoyGateway` config customization beyond
what `--set` flags handle cleanly, `gwp pair install` will accept a
`--config-values <file>` flag that merges the file under the
`gateway-helm.config.envoyGateway` key before passing to Helm. This avoids
long `--set` chains for complex configurations.

Example:

```yaml
# tr-system-1-config.yaml
logging:
  level:
    default: warn
extensionApis:
...
```bash
gwp pair install 1 --config-values tr-system-1-config.yaml
```

Until this flag is implemented, pass `--set gateway-helm.config.envoyGateway.*`
flags directly or use a full Helm values file with `--set-file` / `-f`.

#### Inspecting the active config on a running pair

```bash
kubectl get configmap envoy-gateway-config \
  -n tr-system-1 \
  -o jsonpath='{.data.envoy-gateway\.yaml}' | yq .
```

`gwp pair status 1` also prints the active `controllerName`, watch namespaces,
and `deploy.type` so operators can verify the computed values were applied.
